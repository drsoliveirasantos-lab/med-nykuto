(function(){
  ...
  var copy = {
    es:{kicker:'DESAFÍO 4.º E · 50 R$ PIX',title:'{score}/{total} respuestas correctas',intro:'Publica con tu perfil verificado en la página del desafío.',identity:'Perfil del desafío',missing:'Guarda primero tu nombre y catraca.',publish:'Sumar mis puntos',publishing:'Publicando…',success:'Resultado publicado. Tus puntos ya cuentan para esta semana.',kept:'Tu mejor resultado de este bloque ya era igual o mejor.',error:'No se pudo publicar ahora. El ejercicio sigue guardado en este teléfono.',ranking:'Abrir desafío y clasificación',register:'Guardar identidad'},
    br:{kicker:'DESAFIO 4.º E · R$ 50 PIX',title:'{score}/{total} respostas corretas',intro:'Publique com seu perfil confirmado na página do desafio.',identity:'Perfil do desafio',missing:'Primeiro salve seu nome e catraca.',publish:'Somar meus pontos',publishing:'Publicando…',success:'Resultado publicado. Seus pontos já contam nesta semana.',kept:'Seu melhor resultado deste bloco já era igual ou melhor.',error:'Não foi possível publicar agora. O exercício continua salvo neste telefone.',ranking:'Abrir desafio e classificação',register:'Salvar identidade'}
  };
  ...
  function readProfile(){var profile={};try{profile=JSON.parse(localStorage.getItem(PROFILE_KEY)||'{}')||{};}catch(error){}if(!PLAYER_ID.test(String(profile.playerId||'')))profile.playerId=createPlayerId();profile.displayName=String(profile.displayName||profile.nickname||'').slice(0,60);profile.studentIdMasked=String(profile.studentIdMasked||'').slice(0,20);profile.accessToken=ACCESS_TOKEN.test(String(profile.accessToken||''))?profile.accessToken:'';delete profile.nickname;delete profile.studentId;saveProfile(profile);return profile;}
  ...
  function buildPanel(result){
    ...
    var form=node('form','class-practice-publish-form');var field=node('div','class-practice-publish-field');field.appendChild(node('span','',text('identity')));field.appendChild(node('strong','',profile.displayName?(profile.displayName+' · '+profile.studentIdMasked):text('missing')));var submit=node('button','class-practice-publish-button',profile.accessToken?text('publish'):text('register'));submit.type=profile.accessToken?'submit':'button';form.appendChild(field);form.appendChild(submit);panel.appendChild(form);
    ...
    if(!profile.accessToken){submit.addEventListener('click',function(){window.location.href='comunidade.html#profileTitle';});return;}
    form.addEventListener('submit',function(event){event.preventDefault();submit.disabled=true;submit.textContent=text('publishing');setStatus(status,'','');fetch(API_URL,{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({action:'score',class:'s4-e',playerId:profile.playerId,accessToken:profile.accessToken,courseId:result.courseId,moduleId:result.moduleId,correct:result.correct,total:result.total})}).then(function(response){return response.json().catch(function(){return{};}).then(function(data){if(!response.ok)throw new Error(data.code||'request_failed');return data;});}).then(function(data){setStatus(status,'success',text(data.saved?'success':'kept'));}).catch(function(){setStatus(status,'error',text('error'));}).finally(function(){submit.disabled=false;submit.textContent=text('publish');});});
  }
