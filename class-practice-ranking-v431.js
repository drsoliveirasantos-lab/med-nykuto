(function(){
  ...
  var copy = {
    es:{kicker:'DESAFÍO 4.º E · 50 R$ PIX',title:'{score}/{total} respuestas correctas ya realizadas',intro:'Un solo clic sincroniza juntos tus QCM, verdadero/falso y casos ya respondidos. La clasificación sigue siendo provisional hasta la verificación manual.',identity:'Perfil público del desafío',missing:'Guarda primero tu nombre completo, catraca completa y confirmación del 4.º E.',publish:'Sumar todo mi progreso',publishing:'Publicando…',success:'Todo tu progreso realizado fue sincronizado. Tus puntos ya cuentan provisionalmente esta semana.',kept:'Tus mejores resultados ya eran iguales o mejores.',closed:'El desafío cerró el domingo a las 20:00, hora de Paraguay. El ejercicio sigue guardado, pero ya no cambia la clasificación.',error:'No se pudo publicar todo ahora. Los ejercicios siguen guardados en este teléfono.',ranking:'Abrir desafío y clasificación',register:'Guardar identidad'},
    br:{kicker:'DESAFIO 4.º E · R$ 50 PIX',title:'{score}/{total} respostas corretas já realizadas',intro:'Um único toque sincroniza juntos seus QCMs, verdadeiro/falso e casos já respondidos. A classificação continua provisória até a verificação manual.',identity:'Perfil público do desafio',missing:'Primeiro salve seu nome completo, catraca completa e confirmação do 4.º E.',publish:'Somar todo o meu progresso',publishing:'Publicando…',success:'Todo o seu progresso realizado foi sincronizado. Seus pontos já contam provisoriamente nesta semana.',kept:'Seus melhores resultados já eram iguais ou melhores.',closed:'O desafio encerrou domingo às 20:00, horário do Paraguai. O exercício continua salvo, mas não altera mais a classificação.',error:'Não foi possível publicar tudo agora. Os exercícios continuam salvos neste telefone.',ranking:'Abrir desafio e classificação',register:'Salvar identidade'}
  };
  ...
  function aggregateScore(result){return{correct:Number(result.aggregateCorrect!=null?result.aggregateCorrect:result.correct)||0,total:Number(result.aggregateTotal!=null?result.aggregateTotal:result.total)||0};}
  function scoresToPublish(result){return Array.isArray(result.scores)&&result.scores.length?result.scores:[result];}
  function publishScore(profile,result){
    return fetch(API_URL,{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({action:'score',class:'s4-e',playerId:profile.playerId,accessToken:profile.accessToken,courseId:result.courseId,moduleId:result.moduleId,correct:result.correct,total:result.total})})
  ...
  function buildPanel(result){
    ...
    var aggregate=aggregateScore(result);var profile=readProfile();var ready=profileReady(profile);var panel=node('section','class-practice-publish');panel.appendChild(node('span','practice-eyebrow',text('kicker')));panel.appendChild(node('h5','',text('title',{score:aggregate.correct,total:aggregate.total})));panel.appendChild(node('p','',text('intro')));
    ...
    form.addEventListener('submit',function(event){
      ...
      Promise.all(scoresToPublish(result).map(function(score){return publishScore(profile,score);}))
        .then(function(results){setStatus(status,'success',text(results.some(function(data){return data&&data.saved!==false;})?'success':'kept'));})
        .catch(function(error){setStatus(status,'error',text(error.code==='challenge_closed'?'closed':/^(?:identity_required|identity_ineligible)$/.test(error.code||'')?'missing':'error'));})
