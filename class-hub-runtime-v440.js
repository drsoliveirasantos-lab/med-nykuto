(function(){
  ...
  function noticeImage(notice){
    ...
    try{
      ...
      var frame=el('figure','notice-media'),link=el('a','notice-media-link'),image=el('img'),description=String(notice.imageAlt||notice.image_alt||(attachment&&attachment.title)||notice.title||noticeText('Imagen del aviso','Imagem do aviso')).trim();link.href=parsed.href;link.target='_blank';link.rel='noopener noreferrer';link.referrerPolicy='no-referrer';link.setAttribute('aria-label',noticeText('Abrir imagen: ','Abrir imagem: ')+description);image.src=parsed.href;image.alt=description;image.loading='lazy';image.decoding='async';image.referrerPolicy='no-referrer';image.addEventListener('error',function(){if(frame.parentElement)frame.parentElement.classList.remove('has-image');frame.remove();});link.appendChild(image);frame.appendChild(link);return frame;
