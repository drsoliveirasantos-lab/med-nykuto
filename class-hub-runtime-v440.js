(function(){
  ...
  function loadDriveCatalog(){return fetch('/data/drive-files.json',{headers:{accept:'application/json'}}).then(function(response){if(!response.ok)throw new Error('catalog-offline');return response.json();}).then(function(data){return {files:Array.isArray(data.files)?data.files:[],complete:true};}).catch(function(){return {files:[],complete:false};});}
  ...
  function loadPublic(){var publicRequest=fetch(API+'&resource=public',{headers:{accept:'application/json'}}).then(function(response){if(!response.ok)throw new Error('offline');return response.json();}).then(function(data){return {data:data,complete:true};}).catch(function(){return {data:fallbackPublic(),complete:false};});return Promise.all([publicRequest,loadDriveCatalog()]).then(function(results){var publicResult=results[0],catalogResult=results[1],data=publicResult.data,catalogFiles=catalogResult.files,fallback=fallbackPublic(),apiFiles=Array.isArray(data.files)?data.files:[];publicData={notices:(Array.isArray(data.notices)?data.notices:fallback.notices).filter(noticeIsCurrent).sort(noticeSort),tasks:Array.isArray(data.tasks)?data.tasks:fallback.tasks,activities:Array.isArray(data.activities)?data.activities:fallback.activities,groups:Array.isArray(data.groups)?data.groups:[],members:Array.isArray(data.members)?data.members:[],files:apiFiles,dates:Array.isArray(data.dates)?data.dates:[]};updateStamp(data.contentUpdatedAt||'2026-08-28T20:28:00-03:00');renderNotices();renderLiveTasks();renderDynamicResources();renderGroups();window.dispatchEvent(new CustomEvent('mednykuto:class-public-data',{detail:{files:mergeClassFiles(catalogFiles,apiFiles),complete:publicResult.complete&&catalogResult.complete}}));});}
