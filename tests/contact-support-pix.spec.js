...
test('contact form prepares a local message without navigating away', async ({ page }) => {
  ...
  await page.locator('input[name="email"]').fill('qa@med.test');
  ...
  await expect(page.locator('#contactFormFallbackV358 textarea')).toHaveValue(/Message de test automatique/i);
