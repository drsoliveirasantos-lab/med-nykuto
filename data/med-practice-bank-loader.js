/* v321 — Lazy practice bank loader
   ...
   v321 chains count-safe quality patches after the corresponding bank file.
*/
(function(){
  "use strict";
  ...
  for(var i=0; i<wanted.length; i++){
    ...
    if(!(window.MED_PRACTICE_BANK.byCourse && window.MED_PRACTICE_BANK.byCourse[id])){
      document.write('<script src="data/' + file + '?v=321"><\/script>');
    }
    var patches = patchFiles[id] || [];
    for(var j=0; j<patches.length; j++){
      document.write('<script src="data/' + patches[j] + '?v=321"><\/script>');
    }
  }
