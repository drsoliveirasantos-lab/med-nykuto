/* v304 — Med Nykuto lightweight QCM picker layer.
   QCM rule: app.bundle.js owns questions, answers, progress and navigation.
   This file only adds the course/module picker. It must not mirror progress, observe QCM rerenders,
   repaint the QCM card, or create a progress/loading bar between questions. */
(function(){
  'use strict';
  var SITE_VERSION = 'v304-qcm-picker-only-no-progress-mirror';
  ...
  function injectQcmStyle(){
    if(document.getElementById('qcmCleanPickerStyleV304')) return;
    ['qcmCleanPickerStyleV303','qcmCleanSkipStyle'].forEach(function(id){var old=document.getElementById(id);if(old)old.remove();});
    var st=document.createElement('style');
    st.id='qcmCleanPickerStyleV304';
    st.textContent = [
      ...
      '.premium-progress{display:none!important;visibility:hidden!important;height:0!important;max-height:0!important;overflow:hidden!important;margin:0!important;padding:0!important;border:0!important}',
  ...
  function ensureQcmPicker(){
    if(!isQcm()) return;
    var oldBar = document.querySelector('.premium-progress');
    ...
    window.__MED_NYKUTO_STABILITY__ = {version:SITE_VERSION, defaultLang:'es', protectedDataFiles:true, qcmPickerOnly:true, noProgressMirror:true};
  }
