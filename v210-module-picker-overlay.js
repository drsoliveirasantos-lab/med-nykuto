/* v300 — Med Nykuto stability overlay: default ES, logo-home fix, Biofísica soon, Cloudflare forms, data guards. */
(function(){
  'use strict';
  ...
  function enhanceCatalog(){
    ...
    Array.from(grid.querySelectorAll('.course-card')).forEach(function(card,i){
      ...
      if(isSoonCourse(c)){
        ...
        card.querySelectorAll('.module-count,.course-count,.stat,.count').forEach(function(el){if(/0\s*module/i.test(textOf(el)))el.textContent=L('soon');});
      } else {
  ...
  function progressNumbers(){var s=textOf(document.querySelector('.practice-focus-section')||document.body),m=s.match(/\b(\d{1,2})\s*\/\s*(20)\b/),c=m?Math.max(1,Math.min(20,parseInt(m[1],10))):1,total=m?parseInt(m[2],10):20;return{c:c,total:total,p:Math.round(c/total*100)};}
  ...
  function collapseWeakness(){var root=document.querySelector('.practice-focus-section')||document.body;Array.from(root.querySelectorAll('section,article,div')).forEach(function(el){if(el.closest('.single-question-card,.premium-progress,.mc-picker-shell'))return;var s=textOf(el);if(s.length>45&&s.length<1700&&/Mes points faibles|Puntos débiles|Pontos fracos|ADAPTATIF|ADAPTATIVO/i.test(s)&&/réponses|respuestas|respostas|erreurs|errores|je ne sais pas|no sé|não sei/i.test(s)){el.classList.add('premium-weak-hidden','premium-weak-removed');el.style.display='none';el.setAttribute('aria-hidden','true');}});document.querySelectorAll('.premium-weak-summary,.premium-weak-summary-v2').forEach(function(el){el.remove();});}
  ...
  function patchBrandAndStaticSpanish(){
    ...
    var replacements=[['Med Cursos','Med Nykuto'],['RÉVISION IMMÉDIATE','REVISIÓN INMEDIATA'],['Médecine · révision structurée','Medicina · revisión estructurada'],['Commencer à réviser','Comenzar a revisar'],['Voir tous les modules','Ver todos los módulos'],['Revoir mes erreurs','Revisar mis errores'],['Projet gratuit · soutien libre','Proyecto gratuito · apoyo voluntario'],['Soutenir le projet','Apoyar el proyecto'],['Copier le code Pix','Copiar código Pix'],['Matières présentes','Materias presentes'],['Matières','Materias'],['Modules intégrés','Módulos integrados'],['Progression totale','Progreso total'],['Réinitialiser','Reiniciar'],['Projet ouvert','Proyecto abierto'],['Bibliothèque médicale organisée pour réviser plus vite.','Biblioteca médica organizada para revisar más rápido.'],['À propos','Acerca de'],['Mentions','Aviso legal'],['cas cliniques','casos clínicos'],['vrai/faux','verdadero/falso'],['0 module','Próximamente'],['Entrenar maintenant','Entrenar ahora'],['Razonar como examen','Razonar como en examen'],['Corriger ce qui bloque','Corregir lo que bloquea']];
  ...
  function patchHomeStatsAndBiofisica(){
    ...
    document.querySelectorAll('.subject-progress-card').forEach(function(card){
      if(/Biof[ií]sica/i.test(textOf(card))){card.classList.add('is-coming-soon','biofisica-soon-card'); var span=card.querySelector('span'); if(span) span.textContent=L('soon'); var pct=card.querySelector('.subject-progress-pct'); if(pct)pct.textContent='—'; var bar=card.querySelector('.mini-progress i'); if(bar)bar.style.width='0%';}
    });
  }
  ...
  function patchLegalNetlifyText(){
    document.querySelectorAll('p,li,span,strong,h1,h2,h3').forEach(function(el){var s=textOf(el); if(!s)return; if(s.indexOf('Netlify Forms')>-1)el.textContent=s.replace('Netlify Forms','Cloudflare Pages');});
  }
  ...
  function dataGuard(){
    ...
    window.__MED_NYKUTO_STABILITY__={version:'v300',defaultLang:DEFAULT_LANG,courseCount:hasCourseData?window.MED_COURSES_DATA.courses.length:0,hasBank:hasBank,protectedDataFiles:true};
  ...
  function fixLogoHomeLink(){
    document.querySelectorAll('a.brand,a.brand-official,.site-header a[href="index.html"]').forEach(function(a){
  ...
  function apply(){ensureDefaultLang();injectQcmStyle();fixLogoHomeLink();enhanceCatalog();enhanceReader();enhanceQcm();patchBrandAndStaticSpanish();patchHomeStatsAndBiofisica();patchContactForms();patchLegalNetlifyText();dataGuard();}
