/* v307 — Compact premium QCM picker with scroll-safe touch handling.
   Fix: scrolling inside the modal no longer selects/closes/reloads. */
(function(){
  'use strict';
  ...
  function handleModalClick(e){
    var inModal=e.target&&e.target.closest&&e.target.closest('#qcmForcePickerModal'); if(!inModal)return;
    if(modalTouchMoved){ e.preventDefault(); e.stopPropagation(); modalTouchMoved=false; return; }
    if(e.target.getAttribute&&e.target.getAttribute('data-qfp-close')==='1'){ e.preventDefault(); e.stopPropagation(); close(); return; }
    var choice=e.target.closest('.qfp-choice'); if(choice){ e.preventDefault(); e.stopPropagation(); go(choice.getAttribute('data-kind'),choice.getAttribute('data-id')); }
  }
  ...
  document.addEventListener('touchstart',function(e){ if(!(e.target.closest&&e.target.closest('#qcmForcePickerModal')))return; var t=e.changedTouches&&e.changedTouches[0]; if(!t)return; msx=t.clientX; msy=t.clientY; modalTouchMoved=false; },{capture:true,passive:true});
  document.addEventListener('touchmove',function(e){ if(!(e.target.closest&&e.target.closest('#qcmForcePickerModal')))return; var t=e.changedTouches&&e.changedTouches[0]; if(!t)return; if(Math.abs(t.clientX-msx)>8||Math.abs(t.clientY-msy)>8)modalTouchMoved=true; },{capture:true,passive:true});
  document.addEventListener('touchend',function(e){ if(!(e.target.closest&&e.target.closest('#qcmForcePickerModal')))return; if(modalTouchMoved){ e.preventDefault(); e.stopPropagation(); setTimeout(function(){modalTouchMoved=false},80); return; } handleModalClick(e); },{capture:true,passive:false});
