/* v312 — Stable catalog module picker for the Materias page.
   Fixes module rows by forcing navigation with location.assign(). */
(function(){
  'use strict';
  ...
  function open(course){
    ...
    mods.forEach(function(mod,i){
      var url = moduleUrl(mod);
      html += '<button class="cmf-row" type="button" data-module-url="'+esc(url)+'"><span>Mód. '+esc(mod.number || i+1)+'</span><strong>'+esc(moduleTitle(mod))+'</strong></button>';
    });
    html += '</div></section>';
    var m = modal();
    m.innerHTML = html;
  ...
  function handleClick(e){
    var moduleRow = e.target && e.target.closest && e.target.closest('#catalogModuleForceModal [data-module-url]');
    if(moduleRow){
      e.preventDefault();
      e.stopPropagation();
      if(e.stopImmediatePropagation) e.stopImmediatePropagation();
      navigateModule(moduleRow.getAttribute('data-module-url'));
      return;
    }
