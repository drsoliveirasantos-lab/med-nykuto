...
async function inspectNotebook(browserType, width, failures, screenshots) {
  ...
  expect(hub.bottomLinks === 5, `${browserType.name()} ${width}px: bottom navigation should contain 5 primary items.`, failures);
  ...
  expect(physiology.dates === 6, `${browserType.name()} ${width}px: Physiology should expose 6 dates.`, failures);
  expect(physiology.selectedDate === '27 AGO.', `${browserType.name()} ${width}px: latest Physiology date is not selected.`, failures);
  expect(/Vías sensitivas, decusación y localización de lesiones/.test(physiology.selectedTitle), `${browserType.name()} ${width}px: selected lesson title is missing.`, failures);
  ...
  expect(physiology.narrativeLessons === 6, `${browserType.name()} ${width}px: not all Physiology dates use narrative courses.`, failures);
  ...
  expect(compactViews.modeMax >= 44 && compactViews.modeMax <= 48, `${browserType.name()} ${width}px: notebook menu should keep 44px touch targets (${compactViews.modeMax}px).`, failures);
