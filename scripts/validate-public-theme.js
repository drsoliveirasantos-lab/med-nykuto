...
for (const file of ['clase.html', 'comunidade.html', 'turma-shell/index.html']) {
  ...
  assert.ok(html.includes('public-theme-v485.js?v=485') && html.includes('public-theme-v485.css?v=485'), `${file} is missing the shared theme assets.`);
  assert.ok(html.indexOf('public-theme-v485.js?v=485') < html.indexOf('public-theme-v485.css?v=485'), `${file} does not apply the saved theme before its theme CSS.`);
}
...
assert.ok(worker.includes("med-nykuto-shell-v485") && worker.includes('/public-theme-v485.css?v=485') && worker.includes('/public-theme-v485.js?v=485'), 'The current service-worker shell does not cache the shared theme assets.');
