#!/usr/bin/env node
/* v364 — validates restored course catalog structure and local assets referenced in markdown.
   Legacy internal strings such as "Med Cursos" are tolerated in restored source data
   because runtime normalization converts them to Med Nykuto before display. */
const fs = require('fs');
...
if(data && Array.isArray(data.courses)){
  ...
  data.courses.forEach(course => {
    ...
    modules.forEach(module => {
      ...
      if(/Med Cursos/i.test(md)) legacyBrandRefs += 1;
