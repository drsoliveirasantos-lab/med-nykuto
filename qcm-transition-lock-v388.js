/* v388 — broad QCM viewport lock.
   ...
   It only freezes the visible viewport around QCM question transitions so late rerenders, focus,
...
(function(){
  ...
  function onBeforeAction(e){
    if(isQcmActionTarget(e.target)) lockViewport(950, e.target);
  }

  function onClick(e){
    if(isQcmActionTarget(e.target)) lockViewport(1150, e.target);
  }
  ...
  function bindDirectButtons(){
    ...
    document.querySelectorAll('#practiceList [data-action="next-question"], #practiceList [data-action="previous-question"], #practiceList .option').forEach(function(el){
      ...
      ['pointerdown','touchstart','mousedown'].forEach(function(type){
        el.addEventListener(type, function(ev){ lockViewport(950, ev.currentTarget); }, true);
      });
      el.addEventListener('click', function(ev){ lockViewport(1150, ev.currentTarget); }, true);
    });
  }
  ...
  document.addEventListener('keydown', function(e){
    if((e.key === 'Enter' || e.key === ' ') && isQcmActionTarget(e.target)) lockViewport(1150, e.target);
  }, true);
