/* v393 — QCM render/scroll stability guard.
   app.bundle.js still owns question rendering, currentIndex and navigation.
   This file only neutralizes the two visible transition artefacts from the master QCM render:
   1) the temporary empty #practiceList clear;
   2) the smooth scroll immediately after Next. */
(function(){
  'use strict';
  var VERSION = 'v393-qcm-empty-clear-and-scroll-guard';
  ...
  function patchTemporaryEmptyClear(){
    patchInnerHTMLOn(Element && Element.prototype, '__medNykutoSkipEmptyQcmRenderV393');
    patchInnerHTMLOn(window.HTMLElement && HTMLElement.prototype, '__medNykutoSkipEmptyQcmRenderV393');
  }

  function shouldCalmScrollTarget(el){
  ...
  function patchQcmSmoothScroll(){
    if(Element.prototype.__medNykutoQcmAutoScrollV393) return;
    var nativeScrollIntoView = Element.prototype.scrollIntoView;
    if(typeof nativeScrollIntoView !== 'function') return;
    Element.prototype.scrollIntoView = function(options){
      if(shouldCalmScrollTarget(this)){
  ...
  function inject(){
    ...
    style.id = 'nextVisibilityV393Style';
    style.textContent = [
