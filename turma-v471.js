(function(){
  ...
  function noticeImage(notice){
    ...
    try{
      ...
      var frame=el('figure','notice-media'),link=el('a','notice-media-link'),image=el('img'),description=String(notice.imageAlt||notice.image_alt||(attachment&&attachment.title)||notice.title||'Imagen del aviso').trim();link.href=parsed.href;link.target='_blank';link.rel='noopener noreferrer';link.referrerPolicy='no-referrer';link.setAttribute('aria-label','Abrir imagen: '+description);image.src=parsed.href;image.alt=description;image.loading='lazy';image.decoding='async';image.referrerPolicy='no-referrer';image.addEventListener('error',function(){if(frame.parentElement)frame.parentElement.classList.remove('has-image');frame.remove();});link.appendChild(image);frame.appendChild(link);return frame;
    }catch(error){return null;}
  }
  function noticeCaption(body,content){
    var details=el('details','notice-caption'),summary=el('summary','notice-caption-summary'),preview=el('span','notice-caption-preview',body||'Consulta la información completa y sus enlaces.'),toggle=el('span','notice-caption-toggle'),more=el('span','notice-caption-more','Ver más'),less=el('span','notice-caption-less','Ver menos');toggle.appendChild(more);toggle.appendChild(less);summary.appendChild(preview);summary.appendChild(toggle);details.appendChild(summary);details.appendChild(content);return details;
  }
